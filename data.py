...
def tokenize(batch: Dict) -> Dict:
    ...
    encoded_inputs = tokenizer(batch["Sentence"].tolist(), return_tensors="np", padding="longest")
...
def preprocess(df: pd.DataFrame, class_to_index: Dict) -> Dict:
    ...
    """
    df["Sentence"] = df.Sentence.apply(clean_text)  # clean text
    df["sent_label"] = df["Sentiment"].map(class_to_index)  # label encoding
    df = df[["Sentence", "sent_label"]]  # rearrange columns
    outputs = tokenize(df)
    return outputs
...
class CustomPreprocessor:
    ...
    def fit(self, ds):
        tags = ds.unique(column="Sentiment")
